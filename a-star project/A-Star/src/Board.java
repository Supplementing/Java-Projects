...
/**
 * @author Mason Herron
 * Board.java creates a board (15x15 tile based environment) and also has methods that print the board, calculate the path, set the f, g, and h values for each node, etc.
 *
 */
...
//creates 23 blocked nodes (which is just over 10% of the total nodes of 225)
		Node[] blockedNodes = new Node[23];
		boolean duplicates = false;

		// creates new blocked (unpathable) nodes using a random number generation for
		// the row and column
		for (int i = 0; i < blockedNodes.length; i++) {
			blockedNodes[i] = new Node((int) getRandom(0, 14), (int) getRandom(0, 14), 1);
		}
...
//do while loop that runs until a path is found(ending node is in closed list) or until it gets to a point that a path cannot be found(the openlist is empty)
		...
		//returns the path if there is one, and returns no path found if a path cannot be found
...
//code below calculates the g,h,and f values for all surrounding squares
		...
		// up-right diagonal
		if (currentNode.getRow() != 0 && currentNode.getCol() != 14) {
		...
		// down-right diagonal
		if (currentNode.getRow() != 14 && currentNode.getCol() != 14) {
			...
			if (((board[currentNode.getRow() + 1][currentNode.getCol() + 1]).getType() != 1)
				...
				if (openList.contains(board[currentNode.getRow() + 1][currentNode.getCol() + 1])) {
					...
					// checking to see if the current g value could be made better by going through the current node
					if ((currentNode.getG() + 14) < tempGValue) {
						Node n = board[currentNode.getRow() + 1][currentNode.getCol() + 1];
						// sets the g value equal to + 10 of the current g value
						n.setG(14 + currentNode.getG());
		...
		// down-left diagonal
		if (currentNode.getRow() != 14 && currentNode.getCol() != 0) {
			if (((board[currentNode.getRow() + 1][currentNode.getCol() - 1]).getType() != 1)
					&& (closedList.contains(board[currentNode.getRow() + 1][currentNode.getCol() - 1]) == false)) {
